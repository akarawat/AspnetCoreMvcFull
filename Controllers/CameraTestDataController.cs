...
// #PDU-Web — Dashboard for Camera Test Data (dbo.mig_CameraTestData),
// migrated from DB1 by SP_BATLNK_MigCameraTestData.
public class CameraTestDataController : Controller
{
  private readonly IConfiguration _configuration;

  public CameraTestDataController(IConfiguration configuration)
  {
    _configuration = configuration;
  }

  // -----------------------------------------------------------------------
  // GET /CameraTestData?series=9&fullscreen=true
  // -----------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------
  // GET /CameraTestData/GetCameraTestData?series=9&flagrange=W
  // SP: SP_GetCameraTestData
  // -----------------------------------------------------------------------
  [HttpGet]
  public JsonResult GetCameraTestData(string? series, string? flagrange, string? dt_from, string? dt_to)
  {
    string connStr = _configuration["ConnectionStrings:connBtBiDataUtilize"];
    var result = new List<CameraTestDataModel>();

    using (SqlConnection conn = new SqlConnection(connStr))
    {
      conn.Open();
      SqlCommand cmd = new SqlCommand("SP_GetCameraTestData", conn);
      ...
      while (rdr.Read())
      {
        ...
        result.Add(new CameraTestDataModel
        {
          ...
          focus = rdr["Focus"] == DBNull.Value ? (double?)null : Convert.ToDouble(rdr["Focus"]),
          needleXPosition = rdr["NeedleXPosition"] == DBNull.Value ? (double?)null : Convert.ToDouble(rdr["NeedleXPosition"]),
          needleYPosition = rdr["NeedleYPosition"] == DBNull.Value ? (double?)null : Convert.ToDouble(rdr["NeedleYPosition"])
        });
      }
      conn.Close();
    }
