...
public class TTAController : Controller
{
  private readonly ILogger<TTAController> _logger;
  private readonly IConfiguration _configuration;
  public TTAController(ILogger<TTAController> logger, IConfiguration configuration)
  {
    _logger = logger;
  ...
  [HttpGet]
  public JsonResult GetTTADashboardOutput(string serial)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
                        .SetBasePath(Directory.GetCurrentDirectory())
                        .AddJsonFile("appsettings.json")
    ...
    try
    {
      ...
      using (sqlCon = new SqlConnection(SqlconString))
      {
        ...
        SqlCommand cmd = new SqlCommand("SP_GetTTADashboardOutput", sqlCon);
        cmd.CommandType = CommandType.StoredProcedure;
  ...
  public JsonResult GetDataTTAAll(string? series, DateTime? dtstart, DateTime? dtend, string? flagrange)
  {
    ...
    List<TTADataModel> result = new List<TTADataModel>();


    using (SqlConnection conn = new SqlConnection(connStr))
    {
      conn.Open();
      SqlCommand cmd = new SqlCommand("SP_GetTTA", conn);
      cmd.CommandType = CommandType.StoredProcedure;
      ...
      while (rdr.Read())
      {
        result.Add(new TTADataModel
        {
          ...
          mig_id = Convert.ToInt32(rdr["mig_id"]),
          CDPdiff = Convert.ToInt32(rdr["CDPdiff"]),
          CDPlower = Convert.ToInt32(rdr["CDPlower"]),
          CDPhigher = Convert.ToInt32(rdr["CDPhigher"]),
          CDPhighest = Convert.ToInt32(rdr["CDPhighest"])
        });
      }
  ...
  [HttpGet]
  public JsonResult GetDataTTAOFfsetAllExcel(string? series, DateTime? dtstart, DateTime? dtend, string? flagrange, string? excel_flag)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
        .SetBasePath(Directory.GetCurrentDirectory())
        .AddJsonFile("appsettings.json")
    ...
    List<TTAOffsetDataModel> result = new List<TTAOffsetDataModel>();


    using (SqlConnection conn = new SqlConnection(connStr))
    {
      conn.Open();
      SqlCommand cmd = new SqlCommand("SP_GetTTAOffset", conn);
      cmd.CommandType = CommandType.StoredProcedure;
      cmd.Parameters.AddWithValue("@series", SqlDbType.VarChar).Value = series;
      cmd.Parameters.AddWithValue("@dtstart", SqlDbType.DateTime).Value = dtstart;
      cmd.Parameters.AddWithValue("@dtend", SqlDbType.DateTime).Value = dtend;
      ...
      while (rdr.Read())
      {
        result.Add(new TTAOffsetDataModel
        {
          id = Convert.ToInt32(rdr["id"]),
          serial = rdr["serial"]?.ToString(),
          productionDate = Convert.ToDateTime(rdr["productionDate"]),
          ...
          series = rdr["series"]?.ToString(),
          modelname = rdr["modelname"]?.ToString(),
          idref = Convert.ToInt32(rdr["idref"]),
          OFSLow = Convert.ToInt32(rdr["OFSLow"]),
          OFShigher = Convert.ToInt32(rdr["OFShigher"]),
          OFShighest = Convert.ToInt32(rdr["OFShighest"])
        });
      }
  ...
  // End Class
  [HttpGet]
  public JsonResult GetTTABoxPlot(string series, string criteria, DateTime? fromDate, DateTime? toDate, string? flagrange)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
        .SetBasePath(Directory.GetCurrentDirectory())
        .AddJsonFile("appsettings.json")
    ...
    {
      conn.Open();
      SqlCommand cmd = new SqlCommand("SP_TTALogBoxPlot", conn);
      cmd.CommandType = CommandType.StoredProcedure;

      //cmd.Parameters.AddWithValue("@fromDate", fromDate);
      //cmd.Parameters.AddWithValue("@toDate", toDate);
      //cmd.Parameters.AddWithValue("@flagrange", flagrange);

      cmd.Parameters.AddWithValue("@series", SqlDbType.VarChar).Value = series;
      cmd.Parameters.AddWithValue("@criteria", SqlDbType.VarChar).Value = criteria;
      ...
      using (SqlDataReader rdr = cmd.ExecuteReader())
      {
        while (rdr.Read())
        {
          result.Add(new
          {
            LogDay = rdr.GetDateTime(0),
            Min = Convert.ToDecimal(rdr[1]),
            Max = Convert.ToDecimal(rdr[2]),
            Q1 = Convert.ToDecimal(rdr[3]),
            Median = Convert.ToDecimal(rdr[4]),
            Q3 = Convert.ToDecimal(rdr[5]),
            Count = Convert.ToInt32(rdr[6])
          });
        }
      }
    }
  ...
  [HttpGet]
  public JsonResult GetTTABoxPlotOffset(string series, string criteria, DateTime? fromDate, DateTime? toDate, string? flagrange)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
        .SetBasePath(Directory.GetCurrentDirectory())
        .AddJsonFile("appsettings.json")
    ...
    {
      conn.Open();
      SqlCommand cmd = new SqlCommand("SP_TTALogBoxPlotOffset", conn);
      cmd.CommandType = CommandType.StoredProcedure;
