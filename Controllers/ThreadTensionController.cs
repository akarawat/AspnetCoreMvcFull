...
public class ThreadTensionController : Controller
{
  private readonly ILogger<ThreadTensionController> _logger;
  private readonly IConfiguration _configuration;
  public ThreadTensionController(ILogger<ThreadTensionController> logger, IConfiguration configuration)
  {
    _logger = logger;
  ...
  [HttpGet]
  public JsonResult GetDataKPITopView(string serial)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
                        .SetBasePath(Directory.GetCurrentDirectory())
                        .AddJsonFile("appsettings.json")
    ...
    try
    {
      ...
      using (sqlCon = new SqlConnection(SqlconString))
      {
        ...
        SqlCommand cmd = new SqlCommand("SP_GetButtonFoot3ADashboardOutput", sqlCon);
        cmd.CommandType = CommandType.StoredProcedure;
  ...
  [HttpGet]
  public JsonResult GetDataFoot3A(string? series, DateTime? dtstart, DateTime? dtend, string? flagrange)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
        .SetBasePath(Directory.GetCurrentDirectory())
        .AddJsonFile("appsettings.json")
    ...
    List<KPIFoot3AModel> result = new List<KPIFoot3AModel>();


    using (SqlConnection conn = new SqlConnection(connStr))
    {
      conn.Open();
      SqlCommand cmd = new SqlCommand("SP_GetButtonFoot3a", conn);
      cmd.CommandType = CommandType.StoredProcedure;
      ...
      while (rdr.Read())
      {
        result.Add(new KPIFoot3AModel
        {
          serial = rdr["serial"]?.ToString(),
          startDate_txt = rdr["startDate_txt"]?.ToString(),
          startDate = Convert.ToDateTime(rdr["startDate"]),
          valueA = Convert.ToDouble(rdr["valueA"]),
          valueB = Convert.ToDouble(rdr["valueB"])
        });
      }
  ...
  [HttpGet]
  public JsonResult GetDataFoot3AExcel(string? series, DateTime? dtstart, DateTime? dtend, string? flagrange, string? excel_flag)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
        .SetBasePath(Directory.GetCurrentDirectory())
        .AddJsonFile("appsettings.json")
    ...
    using (SqlConnection conn = new SqlConnection(connStr))
    {
      ...
      while (rdr.Read())
      {
        if (rdr["serial"]?.ToString().Length > 5)
        {
          result.Add(new KPIFoot3AModel
          {
            serial = rdr["serial"]?.ToString(),
  ...
  [HttpGet]
  public async Task<IActionResult> GetExcelExportFoot3AData(string mas_wo, string mas_item)
  {
    IConfiguration _configuration = new ConfigurationBuilder()
                    .SetBasePath(Directory.GetCurrentDirectory())
                    .AddJsonFile("appsettings.json")
